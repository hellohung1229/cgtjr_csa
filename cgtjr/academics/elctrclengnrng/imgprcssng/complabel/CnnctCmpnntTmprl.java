 ...
 */
public class CnnctCmpnntTmprl extends HOGCornerDetectOptmzd {
    ...
    public void cmpnntLabeling(int myOriginalValues[], int i) {
        int pixelDataValues[] = getTemporalGradientValues();
        ...
        //System.out.println("CnnctCmpnntTmprl : pixelDataValues[i] = "+pixelDataValues[i]);
        
        if (i > 0 && isInBounds1x1(leftIndex, aWidth, aHeight) && (pixelDataValues[leftIndex] & 0x00ffffff) != 0x00000000) {
        ...
        } else if (i > 0 && isInBounds1x1(topIndex, aWidth, aHeight) && (pixelDataValues[topIndex] & 0x00ffffff) != 0x00000000) {
            indexValue = indexValues[topIndex];
            indexValues[i] = indexValue;
            labelColor[i] = rtrvColor(indexValue);
            //System.out.println("CnnctCmpnntLbl,top: labelColor[" + i + "]" + labelColor[i] + ", indexValue=" + indexValue);
            updateBoxHashMap(myOriginalValues,i,labelColor[i]);
        } else if (i > 0 && isInBounds1x1(topRightIndex, aWidth, aHeight) && (pixelDataValues[topRightIndex] & 0x00ffffff) != 0x00000000) {
            indexValue = indexValues[topRightIndex];
            indexValues[i] = indexValue;
            labelColor[i] = rtrvColor(indexValue);
            updateBoxHashMap(myOriginalValues,i,labelColor[i]);
            //System.out.println("CnnctCmpnntLbl,top: labelColor[" + i + "]" + labelColor[i] + ", indexValue=" + indexValue);
        } else if (i > 0 && isInBounds1x1(topLeftIndex, aWidth, aHeight) && (pixelDataValues[topLeftIndex] & 0x00ffffff) != 0x00000000) {
            indexValue = indexValues[topLeftIndex];
            indexValues[i] = indexValue;
            labelColor[i] = rtrvColor(indexValue);
            updateBoxHashMap(myOriginalValues,i,labelColor[i]);            
            //System.out.println("CnnctCmpnntLbl,top: labelColor[" + i + "]" + labelColor[i] + ", indexValue=" + indexValue);
        } else if ((pixelDataValues[i] & 0x00ffffff) != 0x00000000) {
