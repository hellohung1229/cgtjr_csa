 ...
 */
public class YSclFltr extends ImageFilter {
    ...
    public YSclFltr(String myFileName) {
        super(myFileName);
        //int aWidth = getImageWidth();
        //int aHeight = getImageHeight();
        //grayValues = new short[aWidth * aHeight];
        //System.out.println("GrySclFltr: filename = "+myFileName);
    }

    public YSclFltr(int myPixelData[], int myImageWidth, int myImageHeight) {
        //super(myFileName);
    ...
    public void gryFltr3x3(int originalValues[], int i) {
        ...
        if(grayValues == null){
           grayValues = new int[originalValues.length]; 
        }
    ...
    public int gryFltr1x1(int originalValues[], int i) {
        int aValue = 0;               
        try {
            grayValues[i] = ColorSpectra.convertRGBToY(originalValues[i]);
            aValue = grayValues[i];            
        } catch (ArrayIndexOutOfBoundsException aiobe) {
        }
        return aValue;
