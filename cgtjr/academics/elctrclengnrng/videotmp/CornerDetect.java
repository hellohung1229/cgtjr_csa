...
public class CornerDetect extends GrdntFilter
{
   ...
   public void updateTrckrInfo(int i)
   {      
      int x = rtrvXPstn(i);
      int y = rtrvYPstn(i);
      trackerBndry.updtBndry(x, y, 0);
   }
   ...
   public void finish() {
      ...
      /*
      int xMax = (int)trackerBndry.getXMax();
      int yMax = (int)trackerBndry.getYMax();
      int xMin = (int)trackerBndry.getXMin();
      int yMin = (int)trackerBndry.getYMin();

      int xMaxyMaxIndex = rtrvIndex(xMax, yMax);
      int xMinyMaxIndex = rtrvIndex(xMin, yMax);
      int xMaxyMinIndex = rtrvIndex(xMax, yMin);
      int xMinyMinIndex = rtrvIndex(xMin, yMin);
