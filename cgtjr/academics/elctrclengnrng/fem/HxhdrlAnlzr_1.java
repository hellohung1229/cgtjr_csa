...
public class HxhdrlAnlzr_1
{
   ...
   private int lclSz;
   private HxhdrlBss hxhdrlBss;
   private int glblSz;
   private double maxValue;
   private double minValue;

   public HxhdrlAnlzr_1(NdlElmnts myNdlElmnts,HxhdrlBss myHxhdrlBss)
   {
      //hxhdrlBss = new HxhdrlBss();
      hxhdrlBss  = myHxhdrlBss;
   ...
   public void calculate(int ne,int nd,int np)
   {
      ...
      for(int i = 0;i<ne;i++)
      {
         for(int j = 0;j<lclSz;j++)
         {
            ...
            //System.out.println("HxhdrlAnlzr: element = "+i+" xl["+j+"]="+xl[j]+",yl["+j+"]="+yl[j]+",zl["+j+"]="+zl[j]);
         }
         hxhdrlBss.setBssNds(xl[0],xl[6],yl[0],yl[6],zl[6],zl[0]);
         ...
         for(int m=0;m<lclSz;m++)
         {
            ...
            {
               /// cemn = hxhdrlBss.cmptCffcntMtrx1(m,n); ... delete
               ce[m][n] = hxhdrlBss.cmptCffcntMtrx(m,n);
               //System.out.println("HxhdrlAnlzr : ce["+m+"]["+n+"]="+ce[m][n]);
               //System.out.println("HxhdrlAnlzr : cemn="+cemn);
            }
         }
         for(int j=0;j<lclSz;j++)
         {
            ...
            for(int l=0;l<lclSz;l++)
            {
               ...
               //System.out.println("HxhdrlAnlzr:  c["+ir+"]["+ic+"]="+ c[ir][ic]+",ce["+j+"]["+l+"]="+ce[j][l]);
            }
         }
      }
      for(int i=0;i<np;i++)
      {
         ...
         v[glblNmbr] = aValue;
         //System.out.println("HxhdrlAnlzr: prescribe v["+glblNmbr+"]="+v[glblNmbr]);
      }
      ...
      for(int n = 0;n<ni;n++)
      {
         for(int i = 0;i<nf;i++)
         {
            sum = 0.0;
            k = lf[i];
            //System.out.println("HxhdrlAnlzr: k = "+k+",lf["+i+"]="+lf[i]);
            for(int j=0;j<nd;j++)
            {
               if(j == k)
               {
                  ...
                  sum = sum + v[j]*c[k][j];
                  //System.out.println("HxhdrlAnlzr:"+",v["+j+"]="+v[j]);
               }
            }
            v[k] = -1*sum/c[k][k];

           // System.out.println("HxhdrlAnlzr: v["+k+"]="+v[k]+",sum = "+sum+",c["+k+"]["+k+"]="+c[k][k]);
         }
      }
   ...
   /*marked for deletion
   private boolean isPrescibePotential(int myI,double myV[],int myNP,int myNDP[],double myVal[])
   {
      for(int k=0;k<myNP;k++)
      {
         if(myI == myNDP[k])
         {
            //System.out.println("HxhdrlAnlzr : node = "+myI+", NDP["+k+"]="+myNDP[k]+", Val["+k+"]="+myVal[k]);
            myV[myI] = myVal[k];
   ...
   /*Delete
   private void statement130()
   {
      return;
   }
   private void statement120()
   {
      return;
   }*/
