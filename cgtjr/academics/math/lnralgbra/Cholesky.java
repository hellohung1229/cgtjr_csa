...
public class Cholesky {
    ...
    public Cholesky(double a[][]) { //: n(a.nrows()), el(a) {
        int i, j, k;
        el = Matrix.copy(a);
        ...
        n = a.length;
        if (el[0].length != n) {
            System.err.println("need square matrix");
        }
        for (i = 0; i < n; i++) {
            for (j = i; j < n; j++) {
                ...
                if (i == j) {
                    if (sum <= 0.0) {
                        System.err.println("Cholesky failed");
                    }
    ...
    void solve(double b[], double x[]) {
        ...
        if (b.length != n || x.length != n) {
            System.err.println("bad lengths in Cholesky");
        }
        for (i = 0; i < n; i++) {
    ...
    void elmult(double y[], double b[]) {
        ...
        if (b.length != n || y.length != n) {
            System.err.println("bad lengths");
        }
        for (i = 0; i < n; i++) {
            b[i] = 0.;
            for (j = 0; j <= i; j++) {
    ...
    public static void main(String args[]){
        double a[][] = {{4,2,-2},{2,10,2},{-2,2,5}};
        
        Cholesky aCholesky = new Cholesky(a);
        double matrixL[][] = aCholesky.getL();
