...
public class MlclDataLoader implements Loader {

    private static final float smallRadius = .05f;
    private static final float mediumRadius = .1f;
    //private static final float largeRadius  = .05f;
    private static final float largeRadius = 0.8f;
    private float sphereRadius = smallRadius;
    private float cylinderRadius = smallRadius;
    private String theFileName;
    //private PDBMoleculeFile thePDBMoleculeFile;
    //private PDBMolecule thePDBMolecule;
    ...
    public Scene load(URL fileNameURL) {
        ...
        System.out.println("MlclDataLoader: load filename = "+fileName);        
    ...
    public Scene load(MoleculeModel thePDBMolecule, float mySphereRadius, float myCylinderRadius) {
         ...
         * 0.0), 100.0); Color3f ambientColor = new Color3f(1.0f, 1.0f, 1.0f);
         * AmbientLight ambientLightNode = new AmbientLight(ambientColor);
         * ambientLightNode.setInfluencingBounds(bounds);
         * objRoot.addChild(ambientLightNode);
         *
         * Color3f light1Color = new Color3f(1.0f, 1.0f, 1.0f); Vector3f
         * light1Direction = new Vector3f(4.0f, -7.0f, -12.0f); DirectionalLight
        ...
        while (anEnumeration1.hasMoreElements()) {
            ...
            Enumeration lineEnumeration = aPDBMolecule1.processConnections(true);
        ...
        while (anEnumeration3.hasMoreElements()) {
            ...
            while (anEnumeration2.hasMoreElements()) {
                ...
                System.out.println("PDBDataLoader.load() : atom = " + thePDBAtom.getClass().getName() + " color = " + thePDBAtom.getColor() + ", radius = " + mySphereRadius);
                /*
                 * diffuseColor3f = new Color3f(new
                 * Color(thePDBAtom.getColor())); ambientColor3f = new
                 * Color3f(new Color(thePDBAtom.getColor())); emmisiveColor3f =
                 * new Color3f(new Color(thePDBAtom.getColor())); diffuseColor3f
                 * = new Color3f(new Color(thePDBAtom.getColor())); m2 = new
                 * Material(ambientColor3f , emmisiveColor3f , diffuseColor3f ,
                 * specularColor3f, 100.0f); a2 = new Appearance();
                 */
                thePDBAtom.setRadius(mySphereRadius);
                Material aMaterial = new Material();
                aMaterial.setDiffuseColor(aColorFloat[0], aColorFloat[1], aColorFloat[2]);
                aMaterial.setAmbientColor(aColorFloat[0], aColorFloat[1], aColorFloat[2]);
                aMaterial.setEmissiveColor(aColorFloat[0], aColorFloat[1], aColorFloat[2]);
                aMaterial.setSpecularColor(aColorFloat[0], aColorFloat[1], aColorFloat[2]);
                //aMaterial.setShininess(50);
                a2 = new Appearance();
                a2.setMaterial(aMaterial);
    ...
    public void setFlags(int myFlags) {
        flags = myFlags;
    }
}